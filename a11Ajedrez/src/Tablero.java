			...
			for (int y = 0 ; y < 8 ; y++) {
				...
				if (casillas[x][y] != null && casillas[x][y].color==T.tirada) {
					for (Posicion posficha : T.casillas[x][y].posiblesPosiciones(T)) {
						...
						if (posficha.x==xRey && posficha.y==yRey) System.out.println(x+" - "+y);
					}
				}
			}
		}
...
//							T.mover(new Posicion(x,y), posficha);
	...
	public void convertirReina(Tablero T) {
		
		for (int x=0;x<8;x++) {
			if(T.tirada=='B' && T.casillas[0][x]!= null &&T.casillas[0][x].forma.equals("\u265F")) {
				T.casillas[0][x].forma="\u265B";
			}	
		}
		
		for (int x=0;x<8;x++) {
			if(T.tirada=='N' && T.casillas[7][x]!= null && T.casillas[7][x].forma.equals("\u2655")) {
				T.casillas[7][x].forma="\u2655";
			}	
		}
	}
